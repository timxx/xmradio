...
static gboolean
idle_get_cover_image(XmrWindow *window);

static gboolean
idle_update_radio_list(XmrWindow *window);

static gboolean
idle_like_song(XmrWindow *window);
...
static void
task_begin(XmrWindow *window, TaskType task);

static void
task_end(XmrWindow *window, TaskType task);
...
{
	if (progress == 0)
		task_begin(window, Task_Buffering);
	else if (progress == 100)
		task_end(window, Task_Buffering);
...
idle_get_cover_image(XmrWindow *window)
{
	...
	const gchar *url = window->priv->current_song->album_cover;

	task_begin(window, Task_GetCover);
...
idle_update_radio_list(XmrWindow *window)
{
	...
	task_begin(window, Task_UpdateRadio);
	...
	for (i = 0; i < 3; ++i)
	{
		...
		while (p)
		{
			...
			else
			{
				...
				xmr_radio = xmr_radio_new_with_info(uri, radio_info->name, radio_info->url);
				xmr_chooser_append(XMR_CHOOSER(window->priv->chooser[i]), GTK_WIDGET(xmr_radio));
			}
	...
	task_end(window, Task_UpdateRadio);
...
idle_dislike_song(XmrWindow *window)
{
	...
	task_begin(window, Task_Dislike);
...
xmr_window_get_playlist(XmrWindow *window)
{
	...
	task_begin(window, Task_GetPlaylist);
	...
	task_end(window, Task_GetPlaylist);
...
xmr_window_set_track_info(XmrWindow *window)
{
	...
	g_idle_add((GSourceFunc)idle_get_cover_image, window);
...
update_radio_list(XmrWindow *window)
{
	g_idle_add((GSourceFunc)idle_update_radio_list, window);
}
...
xmr_window_play_next(XmrWindow *window)
{
	...
	else
	{
		...
		{
			task_begin(window, Task_Download);
...
like_current_song(XmrWindow *window, gboolean like)
{
	...
	g_idle_add((GSourceFunc)(like ?	idle_like_song : idle_dislike_song), window);
				...
				XmrWindow *window)
{
	...
	if (!xmr_window_playing(window) && g_strcmp0(url, priv->current_song->location) == 0)
	{
		task_end(window, Task_Download);
